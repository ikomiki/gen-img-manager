...
- **シートの確定モデルは「両方とも即時反映」に揃える（ユーザ決定済み）。** 現在 `FilterSheet` は「適用」を押すまで検索せず、`DirectorySheet` はチェック1つで即時反映という別々の作法になっている。加えて `FilterSheet` を適用せず閉じると、クエリ入力欄だけが書き換わって表示中の結果と一致しない状態が残り、画面上にその不一致を示すものが無い。`FilterSheet` から「適用」を無くし、入力に応じて検索する形へ変える。**打鍵ごとに 17,000 件へクエリを投げないよう、入力が落ち着いてから投げる制御（デバウンス）が必須**。レーティングのチップやディレクトリのチェックのような離散的な操作は即座に投げてよい
